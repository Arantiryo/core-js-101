 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(s, value) {
...
function encodeToRot13(s) {
  ...
  return s
    .split('')
    .map((v) => {
      if (ALPHABET_UC.indexOf(v) === -1 && ALPHABET_LC.indexOf(v) === -1) {
        return v;
      }
      const code = v === v.toLowerCase() ? ALPHABET_LC : ALPHABET_UC;
      return code[code.indexOf(v) + 13] || code[code.indexOf(v) + 13 - 26];
    })
    .join('');
}
