...
function timeSpanToString(startDate, endDate) {
  ...
  // prettier-ignore
  return `${pad(dateDiff.getUTCHours().toString(), 2)}:${pad(
    dateDiff.getUTCMinutes().toString(),
    2,
  )}:${pad(dateDiff.getUTCSeconds().toString(), 2)}.${pad(
    dateDiff.getUTCMilliseconds().toString(),
    3,
  )}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const diff = Math.abs(0.5 * (60 * hour - 11 * minute));
  const res = diff > 180 ? 360 - diff : diff;
